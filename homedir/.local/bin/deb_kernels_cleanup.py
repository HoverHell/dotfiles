...
def main():
    ...
    lst = (
        ...
        if re.search(r'^linux-([a-z]+-)+[0-9]', item['name']))
    lst = list(lst)
    versions = sorted(list(
        item['version'].rsplit('.', 1)[0]
        for item in lst
        if re.search('^linux-image-[0-9]', item['name'])
    ))
    latest = versions[-1]
    removable = sorted(list(
        item['name'] for item in lst
        if not item['version'].startswith(current) and
        not item['version'].startswith(latest)
    ))
